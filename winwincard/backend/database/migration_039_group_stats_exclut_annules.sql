-- Migration 039 : group_stats exclut les scans annulés (étape 5)
--
-- CREATE OR REPLACE, SIGNATURE STRICTEMENT IDENTIQUE : group_stats(p_marchand_id
-- uuid) RETURNS jsonb. Jamais une variante (§3.3). Le SEUL changement de corps
-- est l'ajout de « AND sc.annule_le IS NULL » dans le CTE `s` — comme `s`
-- alimente TOUS les indicateurs réseau (scans, clients distincts, récompenses,
-- distribution, non-attribués, retour, mobilité, actifs), ce filtre unique les
-- exclut partout d'un coup. Un scan annulé (migration 038) ne compte plus nulle
-- part dans le dashboard groupe.
--
-- Lecture seule (STABLE), ne touche aucune autre fonction. À exécuter APRÈS 038
-- (la colonne annule_le doit exister). Rejouable.
