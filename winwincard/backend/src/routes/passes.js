...
router.get('/:serialNumber/apple', asyncHandler(async (req, res) => {
  ...
  const [{ data: client }, { data: marchand }] = await Promise.all([
    ...
    supabase.from('marchands')
      .select('id, nom, slug, forfait, langue, type_programme, pass_display_name, how_it_works, couleur_fond, couleur_fond_reward, couleur_pastille_fond, couleur_pastille_contour, couleur_pastille_icone, couleur_label_strip, couleur_barre_principale, couleur_barre_secondaire,couleur_texte, couleur_label, logo_url, logo_reward_url, icon_url, image_strip_url, images_tiers, max_value, display_max_value, notification_titre, notification_message, referral_enabled, referral_bonus_points, telephone, adresse, strip_mode, strip_theme, strip_illustration, strip_produit, strip_vide, stamp_icon, strip_custom_background_url, strip_config_version, strip_label')
