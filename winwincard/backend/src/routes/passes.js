...
router.get('/:serialNumber/apple', asyncHandler(async (req, res) => {
  ...
  const [{ data: client }, { data: marchand }] = await Promise.all([
    ...
    supabase.from('marchands')
      .select('nom, slug, pass_display_name, how_it_works, couleur_fond, couleur_texte, couleur_label, logo_url, icon_url, image_strip_url, images_tiers, max_value, display_max_value, notification_titre, notification_message, referral_enabled, referral_bonus_points, telephone, adresse')
