...
router.post('/', limiterInscription, asyncHandler(async (req, res) => {
  ...
  const { data: marchand, error: errMarchand } = await supabase
    .from('marchands')
    .select('id, nom, slug, forfait, langue, type_programme, max_value, display_max_value, actif, couleur_fond, couleur_fond_reward, couleur_pastille_fond, couleur_pastille_contour, couleur_pastille_icone, couleur_label_strip,couleur_texte, couleur_label, logo_url, image_strip_url, google_logo_url, google_hero_url, images_tiers, how_it_works, referral_enabled, referral_bonus_points, telephone, adresse, strip_mode, strip_theme, stamp_icon, strip_custom_background_url, strip_config_version')
